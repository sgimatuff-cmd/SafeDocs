...
<div class="card border-0 shadow-sm mb-4">
    <div class="card-body py-3">
        <p class="small text-muted mb-2 fw-semibold">Hierarquia de cargos (um utilizador pode ter vários):</p>
        <div class="d-flex flex-wrap gap-2">
            @foreach($cargos as $c)
                <span class="badge {{ $c->corBadge() }} small">{{ $c->nome }} — {{ $c->descricao }}</span>
            @endforeach
        </div>
    </div>
</div>
